...
use crate::{cves::domain::{entities::cve_id::CveId, events::cve_created_event::CveCreatedEvent, repositories::cve_repository::CveRepository}, shared::domain::errors::DomainError};
...
impl<R: CveRepository, E: EventBus> CveDeleter<R, E> {
    pub fn new(cve_repository: Arc<R>, event_bus: Arc<E>) -> CveDeleter<R, E> {
        CveDeleter { repository: cve_repository, event_bus }
    }
    ...
    pub async fn run(
        ...
        let old_cve = self.repository.find_by_id(&id).await
        ...
        let created_event = CveCreatedEvent::new_shared(&id, &old_cve.state, &old_cve.date_published, &old_cve.description);
