...
use crate::{
    breaches::{
        application::breach_query_response::BreachQueryResponse,
        domain::{
            entities::{
                breach_product::BreachProduct, breach_product_version::BreachProductVersion,
...
impl<R: BreachRepository> QueryHandler for FindBreachQueryHandler<R> {
    async fn handle(&self, query: Box<dyn Query>) -> Box<dyn QueryBusResponse> {
        ...
        let res = self
            .finder
            .run(vendor, product, product_version, cve_id)
            .await;
        if res.is_err() {
