...
impl<R: BreachRepository> BreachFinder<R> {
    ...
    pub async fn run(
        ...
        debug!(
            "Finding Breach with CVE: {} for Product: {}:{}:{}.",
            cve_id, vendor, product, product_version
        );
        let res = self
            .repository
        ...
        if res.is_err() {
            debug!(
                "Error finding Breach with CVE: {} for Product: {}:{}:{}.",
                cve_id, vendor, product, product_version
            );
            return Err(res.err().unwrap());
        }
        debug!(
            "Breach with CVE: {} for Product: {}:{}:{}.",
            cve_id, vendor, product, product_version
        );
